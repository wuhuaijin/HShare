# https://github.com/andy-yang-1/DoubleSparse/blob/main/models/triton_kernels/channel.py
import torch
...
@triton.jit
def my_get_label_tensor_kernel(X, channel, Out,
                            stride_x_ld, stride_x_h, stride_x_d,
                            stride_channel_h, stride_channel_c,
                            stride_out_ld, stride_out_h, stride_out_c,
                            HEAVY_CHANNEL_NUM: tl.constexpr):
    ...
    cur_token = tl.program_id(0)
    cur_head = tl.program_id(1)
    selected_head = cur_head * 2
    # selected_head = cur_head

    # load channel idx
    offs_channel = channel + selected_head * stride_channel_h + tl.arange(0, HEAVY_CHANNEL_NUM)
    heavy_channels = tl.load(offs_channel)

    # load X's heavy channel
    offs_X = X + cur_token * stride_x_ld + selected_head * stride_x_h + heavy_channels * stride_x_d
    label_tensor = tl.load(offs_X)
...
def my_get_label_tensor(X, channel, Out, HEAVY_CHANNEL_NUM):
    ...
    my_get_label_tensor_kernel[grid](X, channel, Out,
                                  stride_x_ld, stride_x_h, stride_x_d,
                                  stride_channel_h, stride_channel_c,
                                  stride_out_ld, stride_out_h, stride_out_c,
                                  HEAVY_CHANNEL_NUM)
